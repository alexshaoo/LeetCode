...
public:
    ...
    int backtrack(vector<vector<int>>& grid, int r, int c, int numToMatch, int dir, bool turned) {
        ...
        numToMatch = (numToMatch <= 1 ? 2 : 0);
        int right = 0;
        if (!turned) {
            right = backtrack(grid, r + dirs[dir+1][0], c + dirs[dir+1][1], numToMatch, dir+1, true);
        }
        int straight = backtrack(grid, r + dirs[dir][0], c + dirs[dir][1], numToMatch, dir, turned);
    ...
    int lenOfVDiagonal(vector<vector<int>>& grid) {
        ...
        for (int i = 0; i < grid.size(); ++i) {
            for (int j = 0; j < grid[0].size(); ++j) {
                if (grid[i][j] == 1) {
                    for (int d = 0; d < 4; ++d)
                        ans = max(ans, backtrack(grid, i, j, 1, d, false));
                }
            }
        }
        return ans;
